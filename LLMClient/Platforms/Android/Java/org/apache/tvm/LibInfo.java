...
class LibInfo {
  ...
  // Lazy loading - don't load in static block to avoid conflicts with MAUI runtime
  public static synchronized boolean ensureLibraryLoaded() {
    ...
    loadAttempted = true;
    try {
      Log.i(TAG, "Loading libtvm4j_runtime_packed.so...");
      System.loadLibrary("tvm4j_runtime_packed");
      libraryLoaded = true;
      Log.i(TAG, "libtvm4j_runtime_packed.so loaded successfully!");
    } catch (UnsatisfiedLinkError e) {
      Log.e(TAG, "Failed to load libtvm4j_runtime_packed.so: " + e.getMessage());
      libraryLoaded = false;
    }
    return libraryLoaded;
  }
  ...
  // Native methods - NEW TVM FFI API (Vulkan build)
