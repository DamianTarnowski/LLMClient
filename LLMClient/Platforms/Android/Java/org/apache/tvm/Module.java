...
public class Module extends TVMObject {
  ...
  public Function getFunction(String name, boolean queryImports) {
    // New TVM FFI uses ffi.ModuleGetFunction API
  ...
  public void importModule(Module module) {
    // New TVM FFI uses ffi.ModuleImportModule API
  ...
  public String typeKey() {
    // Not available via direct JNI in bundled runtime
    return "unknown";
  }
  ...
  public static Module load(String path, String fmt) {
    // Try to use runtime.module.loadfile if available
    Function loadFunc = Function.getFunction("runtime.module.loadfile");
    if (loadFunc == null) {
      loadFunc = Function.getFunction("runtime.ModuleLoadFromFile");
    }
    if (loadFunc == null) {
      throw new RuntimeException("Module loading not available in bundled runtime");
    }
    TVMValue ret = loadFunc.pushArg(path).pushArg(fmt).invoke();
    return ret.asModule();
  }
  ...
  public static boolean enabled(String target) {
    Function func = Function.getFunction("runtime.RuntimeEnabled");
    if (func == null) {
      return false;
    }
    TVMValue ret = func.pushArg(target).invoke();
    return ret.asLong() != 0;
  }
}
